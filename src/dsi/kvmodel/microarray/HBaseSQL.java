 ...
 *
 * bg: There are two main types of DB engine. One stores all data together without any index and create index tree in which each leaf node points to a row. The other one creates a clustered index for each table with a primary key. If no primary key is defined, the DB engine internally assigns a unique ID to every row and uses that as the clustered index. If DOT used that the former strategy, it would have to rewrite all the secondary indexes at every region split, when the rows get moved to a different location on a permanent storage cluster. To avoid this cost, DOT uses the values from the primary key as its secondary index’s leaf nodes. That makes the secondary indexes independent of the physical order of the primary key. But it causes another problem. The secondary index lookups are more expensive than primary key lookups, because any secondary index lookup only results in a tuple that can be used to navigate the primary key in the original table. That means one query request will be translated into two query operations when using secondary index for DOT. A better solution may be using multiple collabourating clustered indices to restructure the original table.
 *
